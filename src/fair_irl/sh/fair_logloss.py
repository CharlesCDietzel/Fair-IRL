"""
The Robust Fair-Log-Loss classifiers of Rezaei et al. (2020).

Verbatim copy of `fair_logloss/fair_logloss.py` from the reference
implementation published with "Superhuman Fairness" (Memarrast et al., ICML
2023):

    https://github.com/omidMemari/superhumn-fairness

which in turn vendors the implementation released with "Fairness for Robust
Log Loss Classification" (Rezaei, Fathony, Memarrast, Ziebart; AAAI 2020).

Only these edits were made, none of which touch the algorithm:

* `import pdb` (unused, left over from debugging) was dropped.
* `np.Inf` and `np.NINF` became `np.inf` and `-np.inf`. NumPy 2 removed
  those aliases; the values are identical.
* One method docstring containing LaTeX backslash escapes was made a raw
  string, so that Python stops warning about its invalid escape sequences.
* This docstring was added.

The file is deliberately left in the reference implementation's own formatting
rather than this project's, so that it can be diffed against upstream. Keep it
that way; do not run a formatter over it.

`fair_irl.sh.baselines` wraps these classifiers so that they run on this
project's data pipeline and are evaluated like every other model here.
"""
